...
void filter2D(double *I, int *Isize, double *J, int *dimsJ, double *V, int *Vsize, int kernelratio, int windowratio, double filterstrength, int *block, int *block_size, int ThreadID, int Nthreads, double *icov) {
    ...
    for(y=block[1]; y<=block[3]; y++) {
        ...
        for(x=block[0]+ThreadID; x<=block[2]; x+=Nthreads) {
            ...
            indexV1icov=(xv1+indexV1part1)*kernelarea*6;
            ...
            for (jk=-windowratio; jk<=windowratio; jk++) {
                ...
                for (ik=-windowratio; ik<=windowratio; ik++) {
                    ...
                    distance=0;
                    for(i=0; i<kernelarea; i++) {
                        currV1 = indexV1 + i;
                        currV1icov = indexV1icov + i;
                        currV2 = indexV2 + i;
                        diff[0] = V[currV1] - V[currV2];
                        diff[1] = V[currV1 + ch[1]] - V[currV2 + ch[1]];
                        diff[2] = V[currV1 + ch[2]] - V[currV2 + ch[2]];
                        distance += pow2(diff[0])*icov[currV1icov];
                        distance += pow2(diff[1])*icov[currV1icov + ch[1]];
                        distance += pow2(diff[2])*icov[currV1icov + ch[2]];
                        distance += 2*diff[0]*diff[1]*icov[currV1icov + ch[3]];
                        distance += 2*diff[0]*diff[2]*icov[currV1icov + ch[4]];
                        distance += 2*diff[1]*diff[2]*icov[currV1icov + ch[5]];
                    }
                    distance = pow2(sqrt(distance) - sqrt(2 * Vsize[0] - 1));
                    w=exp(-distance*0.5);
...
void filter3D(double *I, int *Isize, double *J, int *dimsJ, double *V, int *Vsize, int kernelratio, int windowratio, double filterstrength, int *block, int *block_size, int ThreadID, int Nthreads) {
 ...
 void filter_multi_threaded(double **Args){
    ...
    if(image3D==0) {
        filter2D(I, Isize, J, dimsJ, V, Vsize, kernelratio, windowratio, filterstrength, block, block_size, ThreadID, Nthreads, icov);
    }
    else {
