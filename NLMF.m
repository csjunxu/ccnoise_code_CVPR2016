...
%
% options,
%   Options.icov : Noise parameter for every pixel, which is the inverse
%                  of covariance matrix. The dimension should be (M, N, C),
%                  where M, N is the height and width of I, and C is the number
%                  of covariance terms, 6 (R, G, B, RG, RB, GB).
...
if(is2D)
    Ipad = padarray(I,[kernelratio+windowratio kernelratio+windowratio],'replicate'); %,
    icovpad = padarray(icov,[kernelratio+windowratio kernelratio+windowratio],'replicate');
else
...
for i=1:length(block);
    ...
    if(is2D)
        ...
        icovblock=icovpad(block(i).x1:block(i).x2,block(i).y1:block(i).y2,:);
    else
        ...
        if(isa(Ipad,'double'))
            ...
            V=image2vectors_double(double(Iblock),double(kernelratio),double(nThreads), double(~is2D));
            ICOV=image2vectors_double(double(icovblock),double(kernelratio),double(nThreads), double(0));
        else
        ...
        if(isa(Ipad,'double'))
            Iblock_filtered=vectors_nlmeans_double(double(Iblock),double(V),double(kernelratio),double(windowratio),double(filterstrength),double(nThreads),double(ICOV));
        else
            Iblock_filtered=vectors_nlmeans_single(single(Iblock),single(V),single(kernelratio),single(windowratio),single(filterstrength),single(nThreads),single(ICOV));
        end
